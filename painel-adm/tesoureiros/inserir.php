...
$res_c = $pdo->query("select * from tesoureiros where cpf = '$cpf'");
...
if($linhas == 0){
	$res = $pdo->prepare("INSERT into tesoureiros (nome, cpf, telefone, email, turno) values (:nome, :cpf, :telefone, :email, :turno)");
	...
	$res->bindValue(":telefone", $telefone);
	$res->bindValue(":email", $email);
	$res->bindValue(":cargo", 'Tesoureiro');
