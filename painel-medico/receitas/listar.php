...
$res_paciente = $pdo->query("SELECT * from pacientes where cpf = '$cpf_paciente'");
	...
	if($cpf_paciente != "todos"){
		$res = $pdo->query("SELECT * from consultas where (data >= '$dataInicial' and data <= '$dataFinal') and medico = '$id_medico' and paciente = '$id_paciente' and receita = 'sim' order by hora asc");
	}else{
		$res = $pdo->query("SELECT * from consultas where (data >= '$dataInicial' and data <= '$dataFinal') and medico = '$id_medico' and receita = 'sim' order by hora asc");
		}
...
for ($i=0; $i < count($dados); $i++) { 
	...
	<a target="_blank" href="rel/rel_receita_class.php?id='.$id.'"><i class="fas fa-file-medical-alt text-info" title="Gerar Receita"></i></a> 
