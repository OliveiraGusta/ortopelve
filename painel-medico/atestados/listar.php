...
$res_paciente = $pdo->query("SELECT * from pacientes where cpf = '$cpf_paciente'");
	...
	$res = $pdo->query("SELECT * from consultas where (data >= '$dataInicial' and data <= '$dataFinal')  and medico = '$id_medico' and atestado != '' order by hora asc");
	}else{
	$res = $pdo->query("SELECT * from consultas where (data >= '$dataInicial' and data <= '$dataFinal') and medico = '$id_medico' and paciente = '$id_paciente' and atestado != '' order by hora asc");}
