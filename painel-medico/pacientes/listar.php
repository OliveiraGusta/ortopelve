...
<table class="table table-sm mt-3">
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from pacientes WHERE medico_resp = '$nome_medico' order by id desc");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
	...
	//TOTALIZAR OS REGISTROS PARA PAGINAÇÃO
		$res_todos = $pdo->query("SELECT * from pacientes WHERE medico_resp = '$nome_medico'");
...
echo ' 
		<tr>
			...
			<td>
				...
				<a title="Editar Ficha" href="index.php?acao='.$pagina.'&funcao=editar&id='.$id.'"><i class="fas fa-edit text-info"></i></a>
				<a title="Excluir Paciente" href="index.php?acao='.$pagina.'&funcao=excluir&id='.$id.'"><i class="far fa-trash-alt text-danger"></i></a>
				

				<a  title="Falar pelo Whatsapp" target="_blank" href="https://api.whatsapp.com/send?phone=55'.$telefone_formatado.'" title="'.$telefone.'"><i class="fab fa-whatsapp text-success"></i></a>

				<a title="Cadastrar Responsável" href="index.php?acao='.$pagina.'&funcao=resp&id='.$id.'"><i class="fas fa-user '.$classe_resp.'"></i></a>
				';
				if($email != null || $email != ""){
				echo '
				<a title="Criar Login" href="index.php?acao='.$pagina.'&funcao=login&id='.$id.'"><i class="fas fa-user-circle text-primaty"></i></a>
';}
echo '
				<a  class="'.$classe_rel.'" target="_blank" title="Última Triagem" href="../painel-medico/rel/rel_triagem_class.php?id='.@$id_triagem.'"><i class="fas fa-book text-secondary"></i></a>
			</td>
		</tr>';

	/*	<a target="_blank" title="Ficha de Paciente" href="../painel-medico/rel/rel_ficha_class.php?id='.$id.'"><i class="fas fa-book text-info"></i></a>
		<a  target="_blank" title="Consultas Anteriores" href="../painel-medico/rel/rel_ficha_consulta_class.php?id='.$id.'"><i class="fas fa-calendar text-secondary"></i></a>*/
	}
